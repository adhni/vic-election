...
- Polish National Electoral Commission 2025 and 2020 presidential voivodeship result archives
- Greek Ministry of Interior / SingularLogic June 2023 and 2019 parliamentary result archives
- Belgian Federal Public Service Interior 2024 and 2019 Chamber result and turnout workbooks
- Eurostat GISCO NUTS boundary archives for Poland and Belgium
...
The Poland presidential views cover both rounds of the 2025 and 2020 elections. The builder downloads checksum-pinned voivodeship CSV archives from the Polish National Electoral Commission and requires candidate totals, valid and invalid ballots, ballots cast, electorate, turnout, local leader, and margin to reconcile independently in all 16 voivodeships. The shared map comes from Eurostat GISCO NUTS geometry. Mazowieckie is reunited from its two statistical NUTS regions so the map matches the election reporting unit. Voivodeships are analytical local-result areas; the president is elected by one nationwide vote.

The Greece parliamentary views cover June 2023 and July 2019. The Ministry of Interior links to the archived SingularLogic election applications, from which the builder checksum-locks the election model, party definitions, official map topology, and all 59 domestic constituency result payloads. Each constituency's party votes reconcile to valid ballots and its valid plus invalid ballots reconcile to ballots cast. The 2023 domestic map deliberately excludes the separate overseas return. The official web map is a schematic inset layout rather than geodetic geometry, and the app labels it accordingly. Constituency seat totals cover 285 seats in 2023 and 288 in 2019; the remaining nationwide list seats are included only in the validated 300-seat Parliament summaries.

The Belgium Federal Chamber views cover 2024 and 2019. The builder parses checksum-pinned result and turnout workbooks from the Federal Public Service Interior, requiring every official party-vote total and all 150 seats to reconcile across 11 multi-member constituencies. The published 2024 East Flanders seat sheet stops before the final party columns and exposes only 148 national seats; the builder reconstructs that constituency's statutory 20-seat D'Hondt allocation from the same official party-vote totals and requires the resulting national party distribution to match the certified 150-seat Chamber.

Belgian counting-canton turnout rows do not separate Brussels from Flemish Brabant cleanly because ballots from facility municipalities cross the constituency reporting boundary. Their combined electorate, valid ballots, invalid ballots, and ballots cast are exact. The builder leaves official constituency party votes and seats untouched and proportionally allocates only electorate and invalid ballots between those two constituencies, while preserving the joint totals and disclosing the treatment in the app. All other constituencies retain their direct official turnout metadata. The map uses Eurostat GISCO NUTS2 geometry joined one-to-one to the 11 constituencies.
...
The generated `data/australia_geography_analysis.json` asset derives electorate-level Labor, Coalition, Greens, independent, and Other first-preference shares from the existing official long-form files for 24 Australian single-member lower-house elections. It does not interpolate electorate totals to suburbs. Exact swing is enabled only for configured pairs using the same legal boundary set: Victoria 2006–2010 and 2014–2018, NSW 2015–2019, Queensland 2020–2024, and federal Victoria 2007–2010. Same-named electorates remain visible in longer history charts with a redistribution warning. Four small 2014 Victorian source-table gaps, together representing less than 0.2% of formal votes in each affected district, remain disclosed as `sourceGap` rather than being assigned to a political bloc.
