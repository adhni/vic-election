...
- U.S. House Clerk official 2024 congressional election statistics
...
Louisiana 2016 and Georgia 2020/2022 use the decisive runoff for the regular seat rather than the earlier inconclusive general-election round. Alaska 2022 uses the official final ranked-choice round as a statewide fallback, and Vermont 2022 uses a statewide fallback because its MIT rows have no county identifiers. Non-geographic ballots and small source-reporting differences remain in the official state view rather than being assigned to a false county. In New Hampshire 2016, the mapped county aggregate narrowly favours Kelly Ayotte while the official state total elects Maggie Hassan; the validator locks this known consequence of the omitted non-geographic component. The builder records a locked state-by-candidate reconciliation signature, and the validator requires exact result/boundary joins, regular-race winner counts, decisive-result spot checks, and the absence of known special-election candidates.
