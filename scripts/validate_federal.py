...
def validate_rows(rows: list[dict[str, str]], expected_divisions: int) -> set[str]:
    ...
    if len(districts) != expected_divisions:
        raise SystemExit(f"Expected {expected_divisions} federal divisions, found {len(districts)}")
...
def validate_against_aec(rows: list[dict[str, str]], aec_raw: Path | None, scope: str) -> None:
    if not aec_raw:
        return
    aec_rows = [row for row in read_aec_csv(aec_raw) if scope_matches_state(scope, row.get("StateAb"))]
...
def main(
    ...
    default_csv: str = "data/federal_2025_au_preferences_long.csv",
    default_boundaries: str = "data/federal_2025_au_division_boundaries.geojson",
    default_expected_divisions: int = 150,
) -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("--csv", type=Path, default=Path(default_csv))
    parser.add_argument("--boundaries", type=Path, default=Path(default_boundaries))
    parser.add_argument("--aec-dop", type=Path)
    parser.add_argument("--expected-divisions", type=int, default=default_expected_divisions)
    ...
    validate_against_aec(rows, args.aec_dop, scope)
    validate_boundaries(args.boundaries, districts)
    print(f"Rows: {len(rows)}")
    print(f"Federal divisions: {len(districts)}")
    print(f"Federal {scope.upper()} validation passed")
