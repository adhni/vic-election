...
GIS_SOURCE = "https://www.aec.gov.au/Electorates/gis/files/Vic-october-2024-esri.zip"
BOUNDARY_NAME_FIXES = {
    "Mcewen": "McEwen",
}
...
def load_lookup(rows: list[dict[str, str]], key: str = "DivisionNm") -> dict[str, dict[str, str]]:
    return {
        row[key]: row
        for row in rows
...
def build_preferences(raw_dir: Path, out_dir: Path) -> tuple[Path, Path]:
    dop = [r for r in read_aec_csv(raw_dir / f"HouseDopByDivisionDownload-{EVENT_ID}.csv") if r["StateAb"] == STATE]
    members = load_lookup(read_aec_csv(raw_dir / f"HouseMembersElectedDownload-{EVENT_ID}.csv"))
    enrolment = load_lookup(read_aec_csv(raw_dir / f"GeneralEnrolmentByDivisionDownload-{EVENT_ID}.csv"))
    informal = load_lookup(read_aec_csv(raw_dir / f"HouseInformalByDivisionDownload-{EVENT_ID}.csv"))
    turnout = load_lookup(read_aec_csv(raw_dir / f"HouseTurnoutByDivisionDownload-{EVENT_ID}.csv"))
    ...
    pref_path = out_dir / "federal_2025_vic_preferences_long.csv"
    summary_path = out_dir / "federal_2025_vic_district_summary.csv"
...
def build_boundaries(shp_path: Path, out_dir: Path) -> Path:
    ...
    out_path = out_dir / "federal_2025_vic_division_boundaries.geojson"
...
def main() -> None:
    ...
    pref_path, summary_path = build_preferences(args.raw_dir, args.out)
    boundary_path = build_boundaries(args.shp, args.out)
