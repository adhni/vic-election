...
RESULTS_URL = f"{BASE}/SG2301/LA/results"
BOUNDARY_ZIP_URL = "https://elections.nsw.gov.au/getmedia/cb9324ee-078f-405b-b4e9-0b95d4e6cefe/2021-gda-94.zip"
...
def discover_districts(session: requests.Session, raw_dir: Path, refresh: bool = False) -> list[DistrictLinks]:
    ...
    if len(districts) != 93:
        raise SystemExit(f"Expected 93 NSW districts, found {len(districts)}")
    return districts
...
def ensure_boundary_shapefile(session: requests.Session, raw_dir: Path, refresh: bool = False) -> Path:
    zip_path = raw_dir / "2021-gda-94.zip"
    extract_dir = raw_dir / "2021-gda-94"
    fetch_bytes(session, BOUNDARY_ZIP_URL, zip_path, refresh=refresh, pause=0)
    shp_path = extract_dir / "2021GDA94" / "StateElectoralDistrict2021_GDA94_region.shp"
    if shp_path.exists() and not refresh:
        return shp_path
...
def build_boundaries(shp_path: Path, out_path: Path, district_names: list[str]) -> None:
    ...
    name_field = next((column for column in gdf.columns if column.lower() == "districtna"), None)
    if not name_field:
...
def main() -> None:
    ...
    parser.add_argument("--raw-dir", type=Path, default=Path("tmp/nsw_2023"))
    parser.add_argument("--out", type=Path, default=Path("data"))
    parser.add_argument("--refresh", action="store_true")
    args = parser.parse_args()

    session = make_session()
    districts = discover_districts(session, args.raw_dir, refresh=args.refresh)
    ...
    write_csv(args.out / "nsw_2023_preferences_long.csv", all_long_rows, LONG_FIELDS)
    write_csv(args.out / "nsw_2023_district_summary.csv", summary_rows, SUMMARY_FIELDS)

    shp_path = ensure_boundary_shapefile(session, args.raw_dir, refresh=args.refresh)
    build_boundaries(shp_path, args.out / "nsw_2023_district_boundaries.geojson", [district.district for district in districts])

    print(f"Wrote {args.out / 'nsw_2023_preferences_long.csv'}")
    print(f"Wrote {args.out / 'nsw_2023_district_summary.csv'}")
    print(f"Wrote {args.out / 'nsw_2023_district_boundaries.geojson'}")
